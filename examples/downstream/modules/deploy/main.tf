...
resource "file_local" "generate_destroy" {
  depends_on = [
    file_local_directory.deploy_path,
    file_local_directory.tf_data_dir,
    # This depends_on is sparce on purpose
    # On destroy, we want this resource to trigger before the state creation resources
    # Neither of these are necessary for create, but create depends on them (why?)
  ]
  directory   = local.tf_data_dir
  name        = "destroy.sh"
  permissions = "0755"
...
resource "file_local_snapshot" "persist_state" {
  depends_on = [
    ...
    file_local.instantiate_tpl_snapshot,
    file_local.generate_files,
    # don't depend on destroy here! We want destroy to be created after this and destroyed before it
...
resource "file_local" "instantiate_state" {
  depends_on = [
    ...
    file_local.instantiate_tpl_snapshot,
    file_local.generate_files,
    # don't depend on destroy here, 
    # we get a different dependency graph for destruction and we need the state to exist for destroy to work
    terraform_data.create,
...
# we wrap create in destroys so that if a failure happens in another resource destroy will still run properly
resource "terraform_data" "destroy_after_create" {
  depends_on = [
    file_local_directory.deploy_path,
    file_local_directory.tf_data_dir,
    file_local.generate_destroy,
    ...
    # this destroy depends on state to exist
  ]
  triggers_replace = {
    trigger = local.deploy_trigger
    dp      = local.tf_data_dir
...
resource "terraform_data" "destroy_end" {
  depends_on = [
    ...
    file_local.generate_create,
    file_local.instantiate_state,
    terraform_data.destroy_after_create,
    file_local.instantiate_outputs,
